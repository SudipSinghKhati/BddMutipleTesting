...
@scenario('../feature\orange.feature', 'Remove oranges from a basket')
def test_remove_oranges_from_a_basket():
    "Remove oranges from a basket"
...
@when(parsers.parse('{more:d} oranges are remove from the basket'))
def oranges_are_remove_from_the_basket(basket, more):
    basket.remove(more)
